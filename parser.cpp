...
parser::parser(main_cube &cube, double &elapse_time, std::ofstream& out) : cube(cube), x(cube.getX()), y(cube.getY()), z(cube.getZ()),
molecule_per_cube(cube.getMoleculePerGasCube()), a(cube.getA()), elapse_time(elapse_time), out(out){
    pressure = 0.;
}
...
void parser::diff_coef_pars() {
    ...
    m << red_left_quantity - red_right_quantity << std::endl;
    out << m.str();
}
